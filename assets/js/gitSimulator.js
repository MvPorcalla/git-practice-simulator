...
import { ERROR_MESSAGES, LOG_TYPES, SYSTEM_MESSAGES } from './gitConfig.js';
...
async function handleGitPush(args) {
    ...
    if (args.length >= 3) {
        if (args[2] === '--set-upstream' || args[2] === '-u') {
            if (args[3]) {
                remoteName = args[3];
                remoteBranch = args[4] || currentBranch;
            } else {
...
document.getElementById('addFileBtn').addEventListener('click', () => {
    ...
    const outsideClickHandler = (e) => {
        ...
        if (!fileName) cancelInput();
        else if (hasError) input.focus();
        else trySaveFile();
    };
